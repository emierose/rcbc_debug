package bluetooth.em.com.projectcountry.view;

import android.widget.EditText;
import android.widget.Spinner;
import android.widget.TextView;

/**
 * Created by Em on 5/6/2016.
 */
public class RegistrationHolder {
    public EditText username;
    public EditText tpass;
    public EditText re_tpass;
    public EditText firstname;
    public EditText middlename;
    public EditText lastname;
    public EditText bdate;
    public Spinner bday_country;
    public Spinner bday_state;
    public Spinner status;
    public EditText mothername;
    public Spinner nationality;
    public EditText tin;
    public EditText sss;
    public EditText passport;
    public EditText acr;
    public EditText cadrno;
    public EditText ccv;
    public EditText p_add;
    public Spinner p_country;
    public Spinner p_state;
    public EditText s_add;
    public Spinner s_country;
    public Spinner s_state;
    public EditText t_add;
    public Spinner t_country;
    public Spinner t_state;
    public EditText residential;
    public EditText office;
    public EditText mobile;
    public EditText p_email;
    public EditText s_email;
    public EditText t_email;
    public EditText branch;
    public EditText code;

    public TextView text;
    public TextView tv_p_country;
    public TextView tv_p_state;
    public TextView tv_s_country;
    public TextView tv_s_state;
    public TextView tv_t_country;
    public TextView tv_t_state;
    public TextView tv_place_country;
    public TextView tv_place_state;
    public TextView tv_nationality;
}
